...
async def _shared_dataset(session: AsyncSession, role: DatasetRole, tag: str):
...
async def test_a_viewer_may_read(session: AsyncSession) -> None:
    dataset, viewer = await _shared_dataset(session, DatasetRole.VIEWER, "read")
...
async def test_a_viewer_may_not_edit(session: AsyncSession) -> None:
    ...
    dataset, viewer = await _shared_dataset(session, DatasetRole.VIEWER, "edit")
...
async def test_an_editor_may_edit(session: AsyncSession) -> None:
    dataset, editor = await _shared_dataset(session, DatasetRole.EDITOR, "ed")
...
async def test_an_owner_member_may_edit(session: AsyncSession) -> None:
    dataset, owner = await _shared_dataset(session, DatasetRole.OWNER, "owm")
...
async def test_every_browser_mutation_refuses_a_viewer(
    ...
    dataset, viewer = await _shared_dataset(session, DatasetRole.VIEWER, "mut")
...
class TestTheRestApiSpeaksTheSameLadder:
    ...
    async def test_a_member_can_read_a_shared_dataset(self, session: AsyncSession) -> None:
        ...
        dataset, viewer = await _shared_dataset(session, DatasetRole.VIEWER, "api-r")
    ...
    async def test_a_viewer_patch_is_403_not_404(self, session: AsyncSession) -> None:
        ...
        dataset, viewer = await _shared_dataset(session, DatasetRole.VIEWER, "api-p")
    ...
    async def test_a_stranger_still_sees_404(self, session: AsyncSession) -> None:
        ...
        dataset, _viewer = await _shared_dataset(session, DatasetRole.VIEWER, "api-s")
    ...
    async def test_an_editor_member_can_rename(self, session: AsyncSession) -> None:
        ...
        dataset, editor = await _shared_dataset(session, DatasetRole.EDITOR, "api-e")
