...
- A person whose browser session was older than collaboration support saw no collaborations, and was told their identity provider had reported none, which it had never been asked. The hub now records when it last read a person's membership, not only what the reading said, so never asked, asked and told nothing, and too old to trust are three different messages. Opening any page takes a reading when there is none or it has gone stale, so nobody has to sign out to get one; a credential that carries no group membership, such as an access token, never takes one and never clears the one you have. Migration `260924_membership_read_at`.

### Added
- A specification can be published to a collaboration instead of to the whole hub. Publishing asks who it is for: **Everyone on this hub**, or one of the collaborations your identity provider reported at your last sign-in. A specification published to a collaboration is immutable and version-gated like any release and datasets can be built on it, but for everyone else it is absent from the specifications list, the profile picker, the explorer, the REST API and the agent tools. The audience is read from the recorded membership, so someone who leaves the collaboration loses sight of it without anyone acting. The version-bump gate stays per account whatever the audience. A client pushing over a token can choose the audience too: `POST /api/specs` takes `audience`, and `GET /api/me` reports the collaborations a publish may name. Migration `260924_spec_audience`; every specification published before it stays visible to everyone, which is what it already was.
...
### Changed
- On a narrow screen the spec builder and the published-spec view stack the sidebar above the canvas, as the dataset page already did.
