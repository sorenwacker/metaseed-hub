...
// Get draft_id from page context (set in base.html)
function getDraftId() {
    return window.draftId || '';
}

// Build API URL with draft_id prefix
...
function deleteEntity(entityName) {
    ...
    fetch(apiUrl(`/entity/${encodeURIComponent(entityName)}`), {
        method: 'DELETE'
    })
    .then(response => {
        if (response.ok) {
...
function updateEntity(event, oldName) {
    ...
    fetch(apiUrl(`/entity/${encodeURIComponent(oldName)}`), {
        ...
        body: `new_name=${encodeURIComponent(newName)}&description=${encodeURIComponent(description)}&ontology_term=${encodeURIComponent(ontologyTerm)}`
    })
    .then(response => {
        if (response.ok) {
...
function toggleLeftSidebar() {
    ...
    const body = document.querySelector('.spec-builder-body');
...
// Initialize existing textareas after HTMX swaps
