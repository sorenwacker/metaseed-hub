...
# Global state for spec builder (per session in production)
_spec_builder_state: SpecBuilderState | None = None


def get_spec_builder_state() -> SpecBuilderState:
    """Get or create spec builder state."""
    global _spec_builder_state
    if _spec_builder_state is None:
        _spec_builder_state = SpecBuilderState()
    return _spec_builder_state


def create_spec_builder_router(
    templates: Jinja2Templates,
    get_state: Callable[[], SpecBuilderState] | None = None,
) -> APIRouter:
    """Create the spec builder router with routes.

    Args:
        templates: Jinja2Templates instance.
        get_state: Optional callable to get state. Uses global if not provided.
    ...
    def get_builder_state() -> SpecBuilderState:
        """Get spec builder state."""
        if get_state:
            return get_state()
        return get_spec_builder_state()
    ...
    async def save_spec_endpoint(request: Request) -> HTMLResponse:
        ...
        # For now, just mark as saved (in production, save to database)
        builder.mark_saved()
        return templates.TemplateResponse(
            request,
            "spec_builder/partials/save_result.html",
