...
async def _loaded_client(
    ...
    Args:
        ...
        on_skip: Receives each stored node the load could not place. Read-only
            callers leave it unset -- the drop is still logged, and
            ``validate_dataset`` reports it -- because a damaged dataset must
            stay inspectable. Editing callers pass a collector, so a mutation
            can refuse rather than rewrite the dataset without those nodes.
    """
    from metaseed import MetaseedClient

    from metaseed_hub.ui.helpers import ensure_dataset_facade

    state = await ensure_dataset_facade(dataset, session, on_skip=on_skip)
    if state.facade is None:
        raise ValueError(
...
def create_mcp_server(name: str = "metaseed-hub") -> FastMCP:
    ...
    register_entity_tools(
        mcp,
        caller=_caller,
        owned_dataset=_owned_dataset,
        editing=_editing,
        loaded_client=_loaded_client,
